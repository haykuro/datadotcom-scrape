...
class Four11DotInfo
{
	...
	public static function scrapeBusiness($args)
	{
		...
		$biz = array(
			'411_url' => self::$business_scrapeURL.$biz_id,
			'411_id'  => $biz_id
		);
		...
		if($content)
		{
			...
			else
			{
				...
				$biz['411_url'] = self::$business_scrapeURL."{$match[1]}/{$match[2]}/".strtoupper(Str::slug($biz['name']))."/{$biz['411_id']}.html";
				...
				for($i = 0; $i < $all_tags->length; $i++)
				{
					...
					else if(strtolower($all_tags->item($i)->tagName) == 'div' && $legend !== false)
					{
						...
						{
							$extra_info[] = array('legend'=>$legend, 'div'=>self::clean_str(array('str'=>$DOM->saveHTML($all_tags->item($i)))));
						}
						$legend = false;
					}
				}
				$biz['extra_info'] = $extra_info;
	 ...
	 * @author Steve Birstok
	**/
	public static function scrapePerson($args)
	{
		...
		$person = array(
			'411_url' => self::$person_scrapeURL.$person_id,
			'411_id'  => $person_id
		);
		...
		if($content)
		{
			...
			else
			{
				...
				$person['411_url'] = self::$person_scrapeURL."{$match[1]}/{$match[2]}/".ucwords($person['family_name']).'-'.ucwords($person['given_name'])."/{$person['411_id']}.html";
